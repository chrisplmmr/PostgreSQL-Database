...
public class GUI_FULL extends JFrame {
    ...
    private static final String[] options = { "Question 1", "Question 2", "Question 3", "Question 4", "Question 5"};
                ...
                try {
                    ...
                    if(id != -1){
                        if(id == 0){ //question 1
                            ...
                            for (String s : arr_input) System.out.println(s + '-');
                            ...
                            if (!q.success) {
                              finalResult = q.status;
                            }
                            else {

                              ArrayList<String> shortest_path = q.solve();
                              if (shortest_path.get(0).equals("NO PATH")) {
                                finalResult = shortest_path.get(0);
                              } else {
                                // test:
                                for (int x = 0; x < shortest_path.size(); x++) {
                                  if (x % 2 == 0) {
                                    ...
                                    // get business name
                                    finalResult += q.runQuery("SELECT name FROM service WHERE business_id = '" + shortest_path.get(x) + "';").get(0);
                                  }
                                  else {
                                    // then it's a user. output user's name
                                    finalResult += q.runQuery("select name from customer where user_id = '" + shortest_path.get(x) + "';").get(0);
                                  }
                            ...
                            if (temp == "") finalResult += "No franchises found";
                            ...
                            sqlStatement = "select count, name, address from (local_restaurant INNER JOIN tip_summary on local_restaurant.business_id = tip_summary.business_id) Where city ='" + cityname + "' ORDER BY count DESC LIMIT 10;";
                            num_columns = 3;
                            finalResult = "--Tips, Business Name, Address--\n";
                            final ResultSet tip_review = stmt.executeQuery(sqlStatement);
                            finalResult += resultSetToString(tip_review, num_columns);
                            if (finalResult.equals("--Tips, Business Name, Address--\n")) {
                              finalResult = "No local businesses found.";
                            }
