...
public class GUI_FULL extends JFrame {
    ...
    private static final String[] options = { "Businesses Details", "Business Locations", "Business Accessability", "Business Hours", "Business Parking", "Customer Details", "Customer Reviews" };
                ...
                try {
                    ...
                    if(id != -1){
                        if(id == 0){
                            ...
                            sqlStatement = "SELECT business_id, name, stars, review_count FROM service";
                            num_columns = 4;
                            fullResult = "--Business_Id, Name, Stars, Review Count--\n";

                        } else if(id == 1){
                            sqlStatement = "SELECT service.name, location.address, location.city, location.state, location.postal_code FROM service INNER JOIN location ON service.business_id = location.business_id";
                            num_columns = 5;
                            fullResult = "--Business Name, Address, City, State, and Postal Code--\n";

                        } else if (id == 2){
                            sqlStatement = "SELECT service.name, attributes.bikeparking, attributes.goodforkids, attributes.byappointmentonly, attributes.wheelchairaccessible FROM service INNER JOIN attributes ON service.business_id = attributes.business_id";
                            num_columns = 5;
                            fullResult = "--Business Name, Bike Parking, Good For Kids, By Appointment Only, Wheelchair Accessible--\n";

                        } else if (id == 3){
                            sqlStatement = "SELECT service.name, hours.sunday, hours.monday, hours.tuesday, hours.wednesday, hours.thursday, hours.friday, hours.saturday FROM service INNER JOIN hours ON service.business_id = hours.business_id";
                            num_columns = 8;
                            fullResult = "--Business Name, Sunday Hours, Monday Hours, Tuesday Hours, Wednesday Hours, Thursday Hours, Friday Hours, Saturday Hours--\n";
                        
                        } else if (id == 4) {
                            sqlStatement = "SELECT service.name, businessparking.garage, businessparking.street, businessparking.validated, businessparking.lot, businessparking.valet FROM service INNER JOIN businessparking ON service.business_id = businessparking.business_id";
                            num_columns = 6;
                            fullResult = "--Business Name, Garage, Street, Validated, Lot, Valet--\n";

                        } else if (id == 5){
                            sqlStatement = "SELECT user_id, name, review_count, yelping_since, fans FROM customer";
                            num_columns = 5;
                            fullResult = "--User ID, Customer Name, Review Count, Yelping Since, Fans--\n"; //mabey add average stars

                        } else if (id == 6){
                            sqlStatement = "SELECT customer.name, review.business_id, review.stars, review.date FROM customer INNER JOIN review ON customer.user_id = review.user_id";
                            num_columns = 4;
                            fullResult = "--Customer Name, Business Reviewed, Stars Given, Review Timestamp--\n";
